...
public class getAllLocationsAction extends ActionSupport implements ServletRequestAware,
	...
	public String execute() throws Exception {
		...
		Date aData = new Date();
		if(timeThreashold < 0)
			timeThreashold = aData.getTime()-30000;
		...
		try{
		   ...
		   if(userInfo != null)
		   {
			   String sql = "select * from (select * from location where groupid = "+activityid+" and time > "+String.valueOf(timeThreashold)+" order by userid,id desc) t group by userid";
			   ...
			   while(rs.next())
			   {
				   ...
				   rows++;
				   userInfo.put(Constant.SESSION_LATESTUPDATE, time);
			   }
			   if(rows == 0)
			   {
				   location = new Location();
