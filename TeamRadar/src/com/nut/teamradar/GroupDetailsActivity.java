...
public class GroupDetailsActivity extends BaseActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		((TextView) findViewById(R.id.group_name)).setText(ActivityName + "(" + group.getAffiliationsCount() + st);
...
//				button.setCompoundDrawables(null, avatar, null, null);
				...
				holder.textView.setText(ActivityName);
								...
								try {
									...
									runOnUiThread(new Runnable() {
										...
										public void run() {
											...
											notifyDataSetChanged();
											String info[] = group.getGroupName().split(":");
											Group grp = TRServiceConnection.getInstance().getGroupByNameAndSub(info[1],info[0]);
											if( grp != null)
											{
												ActivityName  = grp.getName();
											}
											else
											{
												ActivityName = group.getGroupName();
											}
											((TextView) findViewById(R.id.group_name)).setText(ActivityName + "("
			...
			public void run() {
				try {
					...
					runOnUiThread(new Runnable() {
						public void run() {
							String ActivityName="dumy";
							String info[] = Encrypt.GetDecryptString(group.getGroupName()).split(":");
							Group grp = TRServiceConnection.getInstance().getGroupByNameAndSub(info[1],info[0]);
							if( grp != null)
							{
								ActivityName  = grp.getName();
							...
							{
								ActivityName = Encrypt.GetDecryptString(group.getGroupName());
							}
							((TextView) findViewById(R.id.group_name)).setText(ActivityName + "(" + group.getAffiliationsCount()
									+ "人)");
