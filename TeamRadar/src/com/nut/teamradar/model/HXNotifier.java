...
public class HXNotifier {
    ...
    protected final static String[] msg_ch = { "发送消息", "发送图片", "发送语音",
										        "发送位置", "发送视频", "发送文件", "%1 个好友发来 %2 条消息"
										      };
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        try {
        	String displayName = UserUtils.GetUserNameBySub(Encrypt.GetDecryptString(username));
        	if(displayName == null)
        	{
        		displayName = Encrypt.GetDecryptString(username);
        	}
            String notifyText = displayName + " ";
            ...
            if (notificationInfoProvider != null) {
                ...
                {
                    // 璁剧疆鑷畾涔夌殑鐘舵�佹爮鎻愮ず鍐呭
                	String subs[] = customNotifyText.split(":");
                	String Userid = Encrypt.GetDecryptString(subs[0]);
                	String UserName = UserUtils.GetUserNameBySub(Userid);
                	if(UserName == null)
                	{
                		UserName = Userid;
                	}                	
                    notifyText = UserName+customNotifyText.substring(subs[0].length(),customNotifyText.length());
                }
